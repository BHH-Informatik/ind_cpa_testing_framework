...
def encodeToNumpyBitArray(input, bits):
    ...
    if isinstance(input, (bytes, bytearray)):
        return np.unpackbits(input).astype(dtype=float)

    if isinstance(input, int):
        ibytes = bytearray((input).to_bytes(int(math.ceil(bits / 8)), 'big'))
        return np.unpackbits(ibytes).astype(dtype=float)[int(math.ceil(bits / 8)) * 8 - bits:]

    mbytes = bytearray(input, 'utf-8')
    return np.unpackbits(mbytes).astype(dtype=float)
...
class AlgorithmusTesterMT:
    def __init__(self, modelname, samples, batchsize, epochs, description=",",
        ...
        self.models = "models/model-files-{}_{}.keras".format(self.modelname, self.network.getName())
        
        self.logdir = "./logs/" + self.modelname + "/"
    ...
    def decodeFromNumpyArray(self, input):
        ...
        if self.networkDataType == NetworkDataType.BIT:
            ...
            cipher_bytes = np.packbits(cipher_array_rd.astype(dtype=int))
            return cipher_bytes
    ...
    def generate_training_data(self, randomFunc, encryptFunc, mkunique=True):
        ...
        input = np.zeros((self.samples, inputshape[0]), dtype=float)  # ciphertext matrix
        output = np.zeros((self.samples, outputshape[0]), dtype=float)  # plaintext matrix
    ...
    def train(self, lossfunction=losses.mean_squared_error, metrics=[CustomLosses.bitwise_loss, CustomLosses.full_loss],
              earlyStop=False, autotrain=True):
        """
        ...
        #createOutputDir("logs/" + self.modelname+"/")
        print("test " + self.modelname)
        directory = "logs"
        ...
        tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir="{}_{}_{}".format(self.logdir, self.time, self.network.getName()))
        ...
        if autotrain:
            createOutputDir("autotrain/models/")
            model.save("autotrain/models/model-files-{}_{}.keras".format(self.modelname, self.network.getName()))

        else:
            createOutputDir("models/")
            model.save(self.models)
    ...
    def evaluate(self, format_cipher=format_nothing, format_plain=format_nothing):
        ...
        if os.path.exists("autotrain/models"):
            model = load_model("autotrain/models/model-files-{}_{}.keras".format(self.modelname, self.network.getName()),
            ...
            model = load_model("models/model-files-{}_{}.keras".format(self.modelname, self.network.getName()),
                            custom_objects={'bitwise_loss': CustomLosses.bitwise_loss,
                                            'full_loss': CustomLosses.full_loss,
                                            'doublesquare_error': CustomLosses.doublesquare_error})
        print(eg)

        for i in range(reichweite):
